...
- [x] 3. Graph-aware materialization prompt

  - [x] 3.1 Rewrite `materializeToNode()` in `src/lib/intelligence/engine.ts`
...
- [x] 4. Insight-driven node title + summary generation refinements

  - [x] 4.1 Tune prompt temperature and response parsing
  ...
  - [x] 4.2 Validate against art-rock-persona benchmark
...
- [x] 5. LLM semantic edge generation with relationship_type + explanation

  - [x] 5.1 Create `generateSemanticEdge()` helper in `src/lib/intelligence/engine.ts`
  ...
  - [x] 5.2 Integrate with `computeIncrementalEdges()` pipeline
  ...
  - [x] 5.3 Update `add_edge` mutation and `persistMutations` handler
  ...
  - [x] 5.4 Verify edges now have semantic relationship types
...
- [x] 6. Local Graph Synthesis Pass for new node + 3 nearest neighbors

  - [x] 6.1 Create `runGraphSynthesisPass()` function in `src/lib/intelligence/engine.ts`
  ...
  - [x] 6.2 Add `update_node_content` mutation type to `types.ts`
  ...
  - [x] 6.3 Wire synthesis pass into pipeline orchestration
  ...
  - [x] 6.4 Verify synthesis pass improves local subgraph coherence
...
- [x] 7. Benchmarks: Rockstar and art-rock-persona evaluation

  - [x] 7.1 Create Rockstar benchmark at `benchmarks/rockstar-identity.md`
  ...
  - [x] 7.2 Create evaluation utility at `src/lib/intelligence/benchmark.ts`
  ...
  - [x] 7.3 Create debug endpoint `/api/debug/benchmark`
  ...
  - [x] 7.4 Validate both benchmarks score ≥3 on all dimensions
    - Run art-rock-persona benchmark: all dimensions ≥3
    - Run rockstar-identity benchmark: all dimensions ≥3
    - Document final scores
    - _Requirements: 2.6_

- [x] 8. Cleanup, testing, and build verification

  - [x] 8.1 Run `npx tsc --noEmit` — fix any type errors
  ...
  - [x] 8.2 Run `npm run build` — fix any build errors
    - Verify Next.js production build succeeds
    - No new warnings or errors introduced
    - _Requirements: 3.2_

  - [x] 8.3 Verify pipeline end-to-end
  ...
  - [x] 8.4 Verify preservation: segmentation, routing, confidence unchanged
